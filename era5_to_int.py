...
class MetVar:
    def __init__(self, WPSname, ERA5name, ERA5file, beginDateFn, endDateFn, mapProj, isInvariant=False):
...
def find_glade_file(var, validtime):
    ...
    glade_paths = [ '/glade/campaign/collections/rda/data/ds633.6/e5.oper.an.ml/',
...
def write_slab(slab, xlvl, proj, WPSname, hdate, units, map_source, desc):
    ...
    stat = WPSUtils.intermediate.write_next_met_field(5, slab.shape[0], slab.shape[1],
            proj.projType, 0.0, xlvl,
            proj.startLat, proj.startLon, proj.startI, proj.startJ, proj.deltaLat, proj.deltaLon,
            proj.dx, proj.dy, proj.xlonc, proj.truelat1, proj.truelat2, 6371229.0,
            0, v.WPSname, hdate, units, map_source, desc, slab)
...
if __name__ == '__main__':
    ...
    Gaussian = MapProjection(WPSUtils.intermediate.proj_gauss, 89.7848769072, 0.0, 1.0, 1.0, 640.0 / 2.0, 360.0 / 1280.0)
    LatLon = MapProjection(WPSUtils.intermediate.proj_latlon, 90.0, 0.0, 1.0, 1.0, -0.25, 0.25)
        ...
        with Dataset(e5filename) as f:
            ...
            if field_arr.ndim == 2:
                ...
                if v.WPSname == 'SOILGEO':
                    xlvl = 1.0
                write_slab(slab, xlvl, proj, v.WPSname, hdate, units, map_source, desc)
            else:
                for k in range(f.dimensions['level'].size):
                    slab = np.transpose(field_arr[k,:,:])
                    write_slab(slab, float(f.variables['level'][k]), proj, v.WPSname, hdate, units, map_source, desc)
