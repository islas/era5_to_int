...
class RHDiags:
    ...
    from ERA5 T (pressure level temperature) and Q (specific humidity at pressure level) fields.
    """
    ...
    def consider(self, field, xlvl, proj, hdate, slab, intfile):
        ...
        if field == 'SPECHUMD' :
            self.spechumd[xlvl] = slab
        elif field == 'TT' and xlvl != 200100.0 : # How to differentiate from other TT when I need all xlvl????
            self.t[xlvl] = slab
        else:
            return
        ...
        if len( self.t ) == self.levels and len( self.spechumd ) == self.levels :
            for pa in self.spechumd.keys() :
              ...
              t0 = 273.16
              rh = 0.263 * pa * self.spechumd[pa] * (np.exp( 17.67 * ( self.t[pa] - t0 ) / ( self.t[pa] - 29.65 ) ) )
...
if __name__ == '__main__':
    ...
    # Testing
    dont_output.append('TT') 
    dont_output.append('SPECHUMD') 
